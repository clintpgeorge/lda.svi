...
#'
#' @param dtm This may be a DocumentTermMatrix from the tm package, a
#'   simple_triplet_matrix from the slam package, or anything coercible to a
#'   simple_triplet_matrix. This includes a data frame with the document names
#'   in the first column, the terms in the second column, and the number or
#'   appearances in the third column.
#' @param passes How many times we look at each document.
#' @param batchsize The size of the minibatches.
#' @param maxiter The maximum iterations for the "E step" for each document (the
#'   updating of the per-document parameters within each minibatch). The default
#'   of 100 follows the reference implementation in python by the authors.
#' @param eta hyperparameter for the term
#' @param alpha hyperparameter.
#' @param kappa learning rate parameter. Lower values give greater weight to
#'   later iterations. For guaranteed convergence to a local optimum, kappa must
#'   lie in the interval (0.5,1].
#' @param tau_0 learning rate parameter. Higher values reduce the influence of
#'   early iterations.
