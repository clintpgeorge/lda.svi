...
/**
 * Reference: 
 * Online Learning for Latent Dirichlet Allocation. Hoffmann, Blei, Bach (2010). 
 * NIPS. See Algorithm 2 Online variational Bayes for LDA 
 */
void LDA_State::update_minibatch(
	...
	t++; // a class variable for batch index 
  int batchsize = documents.size();
  
  Rcout << "Batch " << t << ": (" << batchsize << " documents) " << " >> "<< endl;
	...
	for (auto doc_id : documents) { // for every document in the batch 
	...
	// Updates word-topic matrix lambda for the batch 
	sufficient_statistics = sufficient_statistics % expElogbeta; // n_tw 
	lambda = lambda * (1 - rho_t) + rho_t * (eta + D * sufficient_statistics / batchsize);
	for (int i = 0; i < K; i++) { // for every topic 
		Elogbeta.row(i) = dirichlet_expectation(lambda.row(i));
		expElogbeta.row(i) = arma::exp(Elogbeta.row(i));
	}
